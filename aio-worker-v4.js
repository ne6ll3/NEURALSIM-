...
const MORPH_THETA = {
  ...
  // FIX: SUBSTANTIVO agora usa 45° apenas como base — o anchorPsi() aplica
  // um offset derivado do hash da forma para distribuir substantivos dentro
  // do quadrante em vez de os empilhar todos em 45°. Isto reduz colisões
  // geométricas que causavam merges indevidos no Dream Cycle.
...
const MorphClassifier = (() => {
  ...
  // FIX: PROPER_RE era demasiado permissivo — qualquer palavra capitalizada
  // passava como nome próprio, incluindo "Oxigénio", "Hidrogénio", "Fogo",
  // "Água" no início de frase. Substituído por lista curada de nomes reais.
  // Acrónimos (todas maiúsculas) continuam a ser classificados pelo ACRONYM_RE
  // acima, que corre primeiro no classify(). Esta lista cobre nomes de pessoas,
  // cidades, países e marcas/termos técnicos comuns — expandível com o
  // comando /name:"X" que ensina o agente que X é um nome próprio.
  const KNOWN_PROPER_NAMES = new Set([
    // Nomes de pessoas — PT/ES/EN mais comuns
    'joão','maria','pedro','ana','luís','luis','carlos','manuel','antonio',
    'antónio','jose','josé','paulo','miguel','sofia','inês','ines',
    'john','mary','james','robert','michael','william','david','richard',
    // Países e cidades lusófonas/relevantes
    'portugal','angola','brasil','moçambique','mozambique','caboverde',
    'timor','guiné','guine','são tome','lisboa','luanda','maputo','porto',
    'brasília','brasilia','coimbra','faro','funchal','aveiro',
    // Outros países e cidades comuns
    'espanha','france','italia','alemanha','estados unidos','china','japão','japao',
    'madrid','paris','roma','berlim','tóquio','toquio','pequim','moscovo',
    // Nomes históricos/científicos (pessoas reais, não conceitos)
    'einstein','newton','darwin','tesla','curie','bohr','feynman','turing',
    'euclides','pitágoras','pitagoras','galileu','kepler','copérnico','copernico',
    'platão','platao','aristoteles','aristóteles','sócrates','socrates','descartes',
    // Marcas e termos técnicos tratados como próprios
    'javascript','typescript','python','nodejs','html','css','linux','windows',
    'android','ios','google','microsoft','apple','amazon','meta','anthropic',
  ]);
  const PROPER_RE = null; // já não é usado — KNOWN_PROPER_NAMES tem prioridade

  // FIX CRÍTICO: VERB_PRES_SUFFIX inclui terminações de UMA letra (o/a/e)
  // que são também as terminações mais comuns de substantivos em português.
  // "fogo","tempo","água","energia","fenómeno" eram TODOS classificados
  // como VERBO_PRESENTE/PASSADO só por terminarem em vogal — um bug
  // fundamental que afectava a extracção de sujeito em praticamente
  // qualquer frase com substantivos comuns. Lista de excepção curada
  // tem prioridade máxima sobre a heurística de sufixo.
  const NOUN_EXCEPTIONS = new Set([
    'fogo','fenómeno','fenomeno','energia','calor','água','agua','vapor','gelo',
    'tempo','espaço','espaco','processo','combustão','combustao','matéria','materia',
    'luz','som','força','forca','massa','volume','pressão','pressao','temperatura',
    'átomo','atomo','molécula','molecula','partícula','particula','onda','campo',
    'água','ar','terra','vento','chuva','neve','gelo','nuvem','céu','ceu',
    'planeta','estrela','sol','lua','universo','átomo','electrão','electron',
    'corpo','objecto','objeto','sistema','estado','evento','acontecimento',
    'causa','efeito','consequência','consequencia','origem','fonte','resultado',
    'vida','morte','nascimento','crescimento','movimento','repouso','equilíbrio','equilibrio',
  ...
  function classify(form, raw) {
    ...
    if (NOUN_EXCEPTIONS.has(form)) return 'SUBSTANTIVO';
    ...
    if (KNOWN_PROPER_NAMES.has(form)) return 'PROPRIO';
...
function anchorPsi(token) {
  ...
  // FIX: para SUBSTANTIVO e PROPRIO, aplica um offset angular individual
  // derivado do hash da forma lexical — distribui conceitos dentro do
  // quadrante em vez de os empilhar todos em θ=45°. Sem isto, "hidrogénio"
  // e "oxigénio" ficam no mesmo ponto geométrico e o Dream Cycle os funde.
  // Range do offset: ±18° (mantém-se dentro do quadrante 0-90°)
  let thetaFinal = theta;
  if (morphClass === 'SUBSTANTIVO' || morphClass === 'PROPRIO') {
    const formStr = token.form || '';
    const hash = formStr.split('').reduce((a, b) => ((a << 5) - a) + b.charCodeAt(0), 0);
    const offset = ((hash % 37) - 18); // ±18°, distribuição pseudo-aleatória estável
    thetaFinal = Math.max(5, Math.min(85, theta + offset));
  }
    ...
    for(const[hash,candidates]of groups){
      ...
      for(let i=0;i<candidates.length;i++){
        for(let j=i+1;j<candidates.length;j++){
          ...
          if (similarity > MEMORY_CONFIG.MERGE_THRESHOLD) {
            // FIX: bloqueia merge se ambos os nós têm proposições distintas —
            // são conceitos semanticamente separados, mesmo que a geometria
            // ψ os aproxime por terem o mesmo morphClass (ex: dois SUBSTANTIVO).
            let blockMerge = false;
            if (typeof PropositionStore !== 'undefined') {
              const aLex = a.node.signature?.lexical?.split(' ')[0] || '';
              const bLex = b.node.signature?.lexical?.split(' ')[0] || '';
              if (aLex && bLex && aLex.toLowerCase() !== bLex.toLowerCase()) {
                const aProps = PropositionStore.queryBySubject(aLex, 0, true).length > 0;
                const bProps = PropositionStore.queryBySubject(bLex, 0, true).length > 0;
                if (aProps && bProps) blockMerge = true;
              }
            }
            if (!blockMerge) { await this.mergeNodes(a.id,b.id,similarity); merged.push(a.id,b.id); }
          }
        }
      }
    }
...
const SemanticTokenizer = (() => {
  ...
  function extract(text) {
    ...
    const rawTokens = text.normalize('NFC').split(/[\s,;.!?()\[\]{}"]+/).filter(t=>t.length>0);
    const tokens = [];
  ...
  function synthesizePhraseGeometry(tokens, contextoAgente) {
    ...
    // NOMINALIZAÇÃO: verbo infinitivo em posição inicial funciona como sujeito
    // "Aquecer é transformar" — "aquecer" é VERBO_INF mas é o sujeito da proposição
    if (!sujeito) {
      const firstVerb = meaningful.find(t =>
        t.psi.morphClass === 'VERBO_INF' || t.psi.morphClass === 'VERBO_PRESENTE'
      );
      if (firstVerb) {
        // Trata o primeiro verbo como sujeito nominal — ajusta θ para quadrante sujeito
        sujeito = {
          ...firstVerb,
          psi: { ...firstVerb.psi, theta: 45 }, // força quadrante de sujeito
        };
        // O predicado é o próximo verbo (se existir) ou o token de maior r
        predicado = meaningful.find(t => t !== firstVerb &&
          (t.psi.morphClass?.startsWith('VERBO') || t.psi.theta >= 90));
      }
    }

    // Fallback final: maior r como sujeito, segundo como predicado
...
const ResponseSynthesizer = (() => {
  ...
  // Detecta língua por heurística
  function detectLang(text) {
    if (!text || text.trim().length === 0) return 'pt'; // default PT
    const ptMarkers = /\b(de|o|a|que|em|para|com|não|uma|um|por|mais|mas|ou|é|são|foi|ser|ter|esta|este|isso|já|ainda|também|porque|então|portanto|aquece|transforma|muda|energia|fogo|água|é|são)\b/gi;
    const enMarkers = /\b(the|is|are|was|were|and|but|or|for|with|this|that|have|has|do|does)\b/gi;
    const ptCount = (text.match(ptMarkers)||[]).length;
    const enCount = (text.match(enMarkers)||[]).length;
    // PT por defeito: só muda para EN se claramente inglês
...
const LexicalSurface = (() => {
  ...
  function surface(node, lang, options = {}) {
    ...
    if (morph.startsWith('VERBO')) {
      // Só conjuga se a forma parece ser um infinitivo (termina em ar/er/ir/or)
      // ou se é um irregular conhecido. Formas já conjugadas são devolvidas como estão.
      const looksLikeInfinitive = /(?:ar|er|ir|or)$/i.test(form);
      const isIrregularPT = form in {
        'ser':1,'ter':1,'estar':1,'fazer':1,'ir':1,'poder':1,
        'saber':1,'querer':1,'vir':1,'dizer':1
      };
      if (looksLikeInfinitive || isIrregularPT) {
        return lang==='pt' ? conjugatePT(form, tense) : conjugateEN(form, tense);
      }
      // Forma já conjugada — devolve como está
      return form;
    }
...
const EpisodicRecall = {
  ...
  push(episode) {
    ...
    // clusters pode ser array ou número dependendo de onde o episódio foi criado
    const rawClusters = episode.semantic?.clusters;
    const clustersArr = Array.isArray(rawClusters) ? rawClusters : [];
    this._cache.push({
      psi:      episode.psiInput,
      input:    episode.input    || '',
      response: episode.response || '',
      cycle:    episode.cycle    || 0,
      concepts: clustersArr.map(c => c && c.centroid).filter(Boolean),
    });
  ...
  injectIntoGraph(recalled, brainRef) {
    ...
    // Calcula dominância actual para escalar o boost
    // Se muitos nós já têm activação alta, reduz o boost episódico
    const activeNodes = Array.from(brainRef.nodes.values()).filter(n => (n.activation||0) > 0.05);
    const totalActivation = activeNodes.reduce((s, n) => s + (n.activation||0), 0);
    const avgActivation   = activeNodes.length > 0 ? totalActivation / activeNodes.length : 0;

    // Boost dinâmico: reduz quando a activação média já é alta
    // Evita que episódios sobrepõem input genuíno
    const dynamicBoost = CFG.EPISODE_BOOST * Math.max(0.2, 1 - avgActivation * 1.5);
    ...
    for (const ep of recalled) {
      ...
      for (const nodeId of epNodes) {
        ...
        if (node) {
          node.activation = Math.min(1, (node.activation || 0) + dynamicBoost);
      ...
      for (let i = 0; i < epNodes.length - 1; i++) {
        brainRef.hebbianConnect(epNodes[i], epNodes[i+1], dynamicBoost * 0.4);
      }
    }
  ...
  async warmup() {
    try {
      ...
      this._cache = withPsi.map(ep => {
        const rawClusters = ep.semantic?.clusters;
        const clustersArr = Array.isArray(rawClusters) ? rawClusters : [];
        return {
          psi:      ep.psiInput,
          input:    ep.input    || '',
  ...
  switch(command) {
    ...
    case 'perceive': {
      ...
      if (decision.type !== 'zap') {
        ...
        const episodePerceive = {
          ...
          semantic: { tokenCount: semanticTokens.length, clusters: semanticClusters, causalPairs: causalPairs, contradictions: contradictions.length }
        };
        ...
        await Persistence.saveEpisode({ cycle, timestamp:Date.now(), decision, metacognition:serializeMeta(), input:inputStr, semantic:{tokenCount:semanticTokens.length,clusters:[],causalPairs:[],contradictions:contradictions.length} });
    ...
    case 'chat': {
      ...
      const episodeChat = {
        ...
        psiInput: psiChat,
        semantic:{tokenCount:cTokens.length, clusters:cClusters, causalPairs:cCausal}
      };
