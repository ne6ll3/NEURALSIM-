...
class NegationGraph {
  ...
  getContradictions() {
    ...
    // Aponta explicitamente para brain.nodes e brain._nodeId
    for (const node of brain.nodes.values()) {
      if (node.polarity === -1 && node.contradicts.size > 0) {
        const posNode = brain.nodes.get(brain._nodeId(node.concept, +1));
        if (posNode) contradictions.push({
          concept: node.concept,
          positiveWeight: posNode.weight, 
          negativeWeight: node.weight,
...
summarize() {
    // Aponta explicitamente para brain.nodes, brain.edges, brain.cycle
    const pos = [...brain.nodes.values()].filter(n => n.polarity === +1).length;
    const neg = [...brain.nodes.values()].filter(n => n.polarity === -1).length;
    
    // Chama a função através do objeto estável brain
    const contradictions = brain.getContradictions();
    
    return { 
      totalNodes: brain.nodes.size, 
      affirmations: pos, 
      negations: neg, 
      contradictions: contradictions.length, 
      topContradictions: contradictions.slice(0, 3), 
  ...
  switch(command) {
    ...
    case 'perceive': {
      ...
      if (decision.type !== 'zap') {
        ...
        if (topToken && (!similar || similar.similarity < 0.4)) {
