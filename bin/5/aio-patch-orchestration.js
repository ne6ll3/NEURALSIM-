...
// ── Comando de diagnóstico das orquestrações corrigido de forma segura ──────────
(function addOrchestrationCommand() {
  // Usar addEventListener garante que não subscrevemos nem partimos o onmessage do v4.js
  self.addEventListener('message', async function(e) {
    if (!e.data || e.data.command !== 'getOrchestration') return;

    // Se as estruturas do cérebro ainda não existirem, ignora temporariamente
    if (typeof brain === 'undefined' || !brain.nodes) return;
    ...
    self.postMessage({
      ...
      goals:         typeof GoalGenerator !== 'undefined'
        ...
        : [],
      beliefs:       typeof SelfModel !== 'undefined' && SelfModel.beliefs ? SelfModel.beliefs.length : 0,
      emergentBeliefs: typeof SelfModel !== 'undefined' && SelfModel.beliefs ? SelfModel.beliefs.filter(b => b.emergent).length : 0,
      X:             typeof XTracker !== 'undefined' ? XTracker.getX() : null,
      attention:     typeof thalamus !== 'undefined' ? thalamus.attentionWeights : null,
      surprise:      typeof Metacognition !== 'undefined' ? Metacognition.surprise || 0 : 0,
    });
  });
})();
