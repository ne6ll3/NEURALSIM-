...
// ============================================================================
const RelationTypes = {
  // IS — EQUIVALÊNCIA pura. Simétrica por natureza: "A é B" tende a
  // implicar proximidade total. Usar apenas quando A e B são
  // verdadeiramente intercambiáveis no contexto, não para categorização.
  ...
  // CATEGORY — pertença ASSIMÉTRICA a uma classe. "Fogo é-um-tipo-de
  // fenómeno" NÃO implica "fenómeno é-um-tipo-de fogo". Distinto de IS
  // precisamente para evitar a poluição que identificaste: múltiplos
  // conceitos podem pertencer à mesma categoria sem se tornarem
  // equivalentes entre si.
  CATEGORY: { code: 'CATEGORY', label: 'é um tipo de', inverse: 'HAS_INSTANCE', combinesWith: ['CATEGORY'] },

  // CONTAINS — composição/continência explícita. "Fogo contém energia"
  // é uma relação parte-todo distinta de posse genérica (HAS) e
  ...
  // com segurança: "A parece-se com B" + "B parece-se com C" NÃO
  // garante "A parece-se com C").
  SIMILAR:  { code: 'SIMILAR',  label: 'é semelhante a', inverse: 'SIMILAR', combinesWith: [] },
...
const BUILTIN_VERB_OPERATORS = {
  ...
  // CONTAINS — continência explícita (distinto de HAS genérico)
  'contém':'CONTAINS', 'conter':'CONTAINS', 'contem':'CONTAINS',
  'engloba':'CONTAINS', 'englobar':'CONTAINS',
...
// ============================================================================
// CONTEXT EXTRACTOR — remove cláusulas de contexto/locativas do início da
// frase antes da extracção SPO. Sem isto, "Em casa a aplicação de T destrói
// tudo" fazia "casa" ser escolhida como sujeito só por ser o primeiro
// substantivo — mesmo sendo apenas o cenário, não o agente da acção.
// A cláusula de contexto é preservada como metadado da proposição (`context`),
// não descartada — fica disponível para /why e para a Camada 4 do ensino
// por camadas (situações específicas).
// ============================================================================
const CONTEXT_MARKERS = new Set([
  'em','na','no','nas','nos','num','numa','duma','dum',
  'durante','quando','se','entre','sob','perante','ante','após','apos',
  'in','at','during','when','if','while',
]);

// Detecta e separa uma cláusula de contexto do início da frase.
// Devolve { contextPhrase, remainingTokens } — remainingTokens é sempre
// um array de tokens (nunca raw string), reconstruído a partir dos tokens
// originais para não perder a estrutura (psi, roles) já calculada.
function stripLeadingContext(tokens) {
  if (!tokens || tokens.length === 0) return { contextPhrase: null, remainingTokens: tokens };

  const first = tokens[0];
  if (!CONTEXT_MARKERS.has((first.form || '').toLowerCase())) {
    return { contextPhrase: null, remainingTokens: tokens };
  }

  // Caso A: há vírgula na sequência de tokens originais (marcada por um
  // token cujo raw contém ',') — tudo antes da vírgula é contexto
  const commaTokenIdx = tokens.findIndex(t => (t.raw || t.form || '').includes(','));
  if (commaTokenIdx !== -1) {
    const contextTokens = tokens.slice(0, commaTokenIdx + 1);
    return {
      contextPhrase:   contextTokens.map(t => t.raw || t.form).join(' ').replace(/,$/, ''),
      remainingTokens: tokens.slice(commaTokenIdx + 1),
    };
  }

  // Caso B: sem vírgula — heurística conservadora. Marcador + stopwords
  // seguintes + o primeiro token de conteúdo imediato são tratados como
  // contexto. Avança no máximo até ao primeiro token não-stop.
  const contextTokens = [first];
  let i = 1;
  while (i < tokens.length && tokens[i].roles?.isStop) { contextTokens.push(tokens[i]); i++; }
  if (i < tokens.length) { contextTokens.push(tokens[i]); i++; }

  return {
    contextPhrase:   contextTokens.map(t => t.raw || t.form).join(' '),
    remainingTokens: tokens.slice(i),
  };
}

// ============================================================================
// COMPOUND NOUN PHRASE DETECTOR — "SUBSTANTIVO de SUBSTANTIVO"
// "A aplicação de calor destrói o gelo" — sem isto, "calor" competia com
// "gelo" pelo papel de objecto, ou desaparecia. Detecta o padrão head+de+
// modificador, remove o complemento da extracção SPO principal (evitando
// que "calor" seja escolhido erradamente como objecto de "destrói"), e
// devolve o modificador como `qualifier` — preservado, não descartado.
//
// HONESTIDADE SOBRE O LIMITE: "de" em português cobre vários sentidos
// (posse: "casa de João"; composição: "garrafa de vidro"; tema/instrumento:
// "aplicação de calor"). Esta heurística NÃO distingue esses sentidos —
// trata-os todos da mesma forma estrutural. A proposição secundária que
// gera fica marcada como 'inferred', não 'taught', precisamente porque a
// relação exacta (HAS vs CONTAINS vs outra) é uma aproximação, não uma
// certeza extraída da frase.
// ============================================================================
function detectCompoundNounPhrase(tokens) {
  if (!tokens || tokens.length < 3) return null;

  for (let i = 0; i < tokens.length - 2; i++) {
    const head = tokens[i];
    const prep = tokens[i + 1];
    const mod  = tokens[i + 2];

    if ((prep.form || '').toLowerCase() !== 'de') continue;

    const headMC = head.psi?.morphClass || '';
    const modMC  = mod.psi?.morphClass  || '';
    const headIsNoun = ['SUBSTANTIVO', 'PROPRIO'].includes(headMC);
    const modIsNoun  = ['SUBSTANTIVO', 'PROPRIO'].includes(modMC);

    if (headIsNoun && modIsNoun) {
      return { head: head.form, modifier: mod.form, headIdx: i, prepIdx: i + 1, modIdx: i + 2 };
    }
  }
  return null;
}

// Remove os tokens da preposição "de" e do modificador da sequência,
// mantendo o "head" — para a extracção SPO principal não competir com o
// modificador pelo papel de objecto/sujeito
function removeCompoundComplement(tokens, compound) {
  if (!compound) return tokens;
  return tokens.filter((t, idx) => idx !== compound.prepIdx && idx !== compound.modIdx);
}
...
const PropositionStore = (() => {
  ...
  function makeProp(subject, predicate, object, relation, opts = {}) {
    return {
      ...
      // Cláusula de contexto/situação extraída da frase original, se existir
      // ("Em casa" em "Em casa, o fogo destrói tudo"). Camada 4 do ensino
      // por camadas — situações específicas, não regras gerais.
      context:        opts.context || null,
      // Modificador de sintagma nominal composto ("aplicação DE calor")
      // — preservado quando detectado, mesmo não fazendo parte do SPO principal
  ...
  // FIX: um token de uma só letra maiúscula (raw) é provavelmente um
  // símbolo/variável real (elemento químico, incógnita) — não deve ser
  // descartado só por ser curto. Ex: "T", "X", "H", "O" em contextos
  // científicos ou algébricos.
  function _isLikelySymbol(t) {
    return t.raw && /^[A-ZÁÉÍÓÚÀÂÊÔ]$/.test(t.raw);
  }

  function _extractSPO(tokens, enrichedTokens) {
    const toks = enrichedTokens || tokens;
    const meaningful = toks.filter(t =>
      !t.roles?.isStop && (t.form?.length > 1 || _isLikelySymbol(t))
    );
    ...
    // FIX: o fallback de sujeito nunca deve escolher o mesmo token já
    // atribuído ao predicado — evita {subject:X, predicate:X} duplicado
    if (!subject && meaningful.length > 0) {
      subject = meaningful.find(t => t !== predicate) || meaningful[0];
    }
  ...
  function teach(tokens, enrichedTokens, opts = {}) {
    ...
    // ── PRIORIDADE 4: remove cláusula de contexto do início da frase ───────────
    // "Em casa, a aplicação de calor destrói o gelo" — sem isto, "casa" seria
    // erradamente escolhido como sujeito por ser o primeiro substantivo.
    const sourceForContext = enrichedTokens || tokens;
    const { contextPhrase, remainingTokens } = stripLeadingContext(sourceForContext);
    let spoTokens = contextPhrase && remainingTokens?.length > 0 ? remainingTokens : tokens;
    let spoEnrichedTokens = contextPhrase && remainingTokens?.length > 0 ? remainingTokens : enrichedTokens;

    // ── PRIORIDADE 5: detecta sintagma nominal composto ("aplicação de calor") ─
    // Remove o complemento "de X" da extracção principal, preservando-o
    // como qualifier. Sem isto, "calor" competia com o objecto real ("gelo")
    // ou desaparecia sem deixar rasto.
    const compound = detectCompoundNounPhrase(spoEnrichedTokens || spoTokens);
    if (compound) {
      spoTokens         = removeCompoundComplement(spoTokens, compound);
      spoEnrichedTokens = removeCompoundComplement(spoEnrichedTokens, compound);
    }
    ...
    const relation = opts.forceRelation && RelationTypes[opts.forceRelation]
    ...
    const prop = makeProp(
      spo.subject, spo.predicate, spo.object, relation,
      {
        ...
        verified:   true,
        context:    contextPhrase || null,
        qualifier:  compound?.modifier || null,
      }
    );
    ...
    console.log('[PropStore] Ensinado:', prop.subject, polSign + prop.relationLabel, prop.object,
      ...
      compound ? '| qualifier:'+compound.modifier : '');
    _lastTaughtProp = prop; // actualiza para o operador "também" elíptico

    // FIX: ensina também uma proposição secundária ligando o head ao
    // modificador do composto — "aplicação" relaciona-se com "calor".
    // Marcada como 'inferred' (não 'taught') porque a heurística "de"
    // não distingue posse/composição/tema — é uma aproximação estrutural,
    // auditável e disputável via /why e /incorrect, não uma certeza.
    if (compound && compound.head !== compound.modifier) {
      const qualTokens = [
        { form: compound.head, roles:{isStop:false,isNeg:false,isCausal:false}, semanticWeight:0.7,
          psi: anchorPsi({form:compound.head,roles:{},semanticWeight:0.7}) },
        { form: 'tem', roles:{isStop:false,isNeg:false,isCausal:false}, semanticWeight:0.7,
          psi: anchorPsi({form:'tem',roles:{},semanticWeight:0.7}) },
        { form: compound.modifier, roles:{isStop:false,isNeg:false,isCausal:false}, semanticWeight:0.7,
          psi: anchorPsi({form:compound.modifier,roles:{},semanticWeight:0.7}) },
      ];
      ...
      if (!existingQual) {
        ...
        _props.set(qualProp.id, qualProp);
        _index(qualProp);
        _persistProp(qualProp);
        console.log('[PropStore] Qualificador inferido:', compound.head, 'tem', compound.modifier, '(conf:0.65)');
      }
    }
  ...
  // ============================================================================
  // TEACH STRUCTURED — entrada de conhecimento já resolvido (JSON de currículo)
  // Percorre o MESMO caminho estrutural que teach(): _hasConflict, makeProp,
  // _index, _persistProp — a única coisa que salta é a extracção de
  // linguagem natural (_extractSPO, detectRelationType), porque os dados já
  // chegam com subject/relation/object resolvidos. Isto é deliberado: dados
  // importados usam source:'taught' e por isso têm de obedecer às mesmas
  // regras de conflito, verificação e persistência que qualquer facto
  // ensinado interactivamente — nenhum atalho na validação.
  //
  // entry: { subject, relation, object, qualifier?, context?, strength?,
  //          polarity? }
  // Devolve: { ok, prop? , conflict?, existingProp?, reason? }
  // ============================================================================
  function teachStructured(entry, opts = {}) {
    if (!entry || !entry.subject || !entry.relation) {
      return { ok: false, reason: 'missing_subject_or_relation' };
    }
    const relationObj = RelationTypes[entry.relation];
    if (!relationObj) {
      return { ok: false, reason: 'unknown_relation:' + entry.relation };
    }
    ...
    // proposições importadas EXACTAMENTE como proposições ensinadas por
    // texto, não como uma via secundária de segunda classe.
    const subjToken = { form: subject, roles: { isStop:false, isNeg: polarity===-1, isCausal:false }, semanticWeight: 0.8 };
    const psiSubject = anchorPsi(subjToken);
    let psiObject = null;
    if (object) {
      const objToken = { form: object, roles: { isStop:false, isNeg:false, isCausal:false }, semanticWeight: 0.8 };
      psiObject = anchorPsi(objToken);
    }

    // Verificação estrutural leve — mesmo sem tokens de frase natural,
    // ainda vale a pena confirmar que sujeito/objecto não colidem por
    // polaridade directa no NegationGraph (proteção real, não decorativa)
    if (typeof PropositionVerifier !== 'undefined') {
      const seq = [
        { form: subject, role: 'subject', polarity, psi: psiSubject },
      ];
      if (object) seq.push({ form: object, role: 'object', polarity, psi: psiObject });
      const verification = PropositionVerifier.verify(seq, 'assertion');
      if (!verification.valid) {
        return { ok: false, reason: 'verification_failed', verification };
      }
    }

    const conflict = _hasConflict(subject, object, relationObj, polarity);
    if (conflict === 'duplicate') {
      const ids = _bySubject.get(subject) || new Set();
      for (const id of ids) {
        const p = _props.get(id);
        if (p && p.relation === relationObj.code && p.polarity === polarity &&
            p.object?.toLowerCase() === (object || '').toLowerCase()) {
          p.fireCount++;
          p.confidence = Math.min(1, p.confidence + 0.05);
          p.lastUsed   = Date.now();
          _persistProp(p);
          return { ok: true, reinforced: true, prop: p };
        }
      }
    }
    if (conflict && conflict.type) {
      return { ok: false, conflict: true, existingProp: conflict.existingProp };
    }

    const prop = makeProp(subject, null, object, relationObj, {
      confidence: entry.strength != null ? Math.max(0, Math.min(1, entry.strength)) : (opts.defaultConfidence || 1.0),
      source:     'taught',
      cycle:      opts.cycle || 0,
      psiSubject, psiObject, polarity,
      verified:   true,
  ...
  function inferTransitive(propA, propB) {
    ...
    let inferredConf = propA.confidence * propB.confidence * negationPenalty;

    // ── Consulta ao AntiPatternRegistry (dependência suave) ───────────────────
    // Antes de finalizar a confiança, verifica se este candidato (sujeito+
    // objecto+relação) se parece, por similaridade estrutural, com uma
    // falha histórica REAL (disputada por humano — nunca por inferência
    // anterior, ver garantia de origem no aio-patch-antipattern.js).
    // Isto é aplicado na CRIAÇÃO da proposição inferida — nunca reescreve
    // confiança de fatos já ensinados por humanos, só torna esta NOVA
    // inferência mais cautelosa quando ecoa um erro já confirmado.
    let antiPatternFlag = null;
    if (typeof AntiPatternRegistry !== 'undefined' && AntiPatternRegistry.checkAgainstHistory) {
      const check = AntiPatternRegistry.checkAgainstHistory({
        subject: propA.subject, object: propB.object, relation: rtA.code,
      });
      if (check && check.matched) {
        inferredConf *= 0.5; // penalidade adicional — eco de falha confirmada
        antiPatternFlag = { failureRef: check.failure, subjSim: check.subjSim, objSim: check.objSim };
        console.log('[PropStore] Inferência ecoa falha histórica — confiança reduzida:',
          propA.subject, rtA.label, propB.object, '| conf ajustada:', inferredConf.toFixed(3));
      }
    }
    ...
    if (antiPatternFlag) prop.antiPatternFlag = antiPatternFlag;
  ...
  function purgeDeprecated(minAgeMs = 300000, maxFireCount = 1) {
    ...
    // FIX: remove também do disco — mesma razão do retract()
    if (purgedIds.length > 0) {
      DB.open().then(() => Promise.all(purgedIds.map(id => DB.propositions.delete(id))))
        .catch(e => console.warn('[PropStore] Falha ao purgar do disco:', e.message));
    }
  ...
  function retract(subjectForm, objectForm) {
    ...
    // FIX: remove também do IndexedDB — sem isto, a proposição "apagada"
    // reaparecia após o próximo reload porque continuava gravada em disco
    // (a persistência real só foi ligada depois desta função ter sido escrita).
    if (removedIds.length > 0) {
      DB.open().then(() => Promise.all(removedIds.map(id => DB.propositions.delete(id))))
        .catch(e => console.warn('[PropStore] Falha ao remover do disco:', e.message));
    }
  ...
  // FIX CRÍTICO: a tabela 'propositions' agora existe no schema (DB v3).
  // Antes deste fix, esta função nunca persistia nada — o PropositionStore
  // inteiro vivia só em memória e desaparecia a cada reload do worker.
  async function _persistProp(prop) {
    try {
      await DB.open();
      await DB.propositions.put(prop);
    } catch(e) {
      console.warn('[PropStore] Falha ao persistir proposição:', e.message);
    }
  }

  // Carrega proposições do IndexedDB no arranque — agora funciona de facto,
  // porque a tabela 'propositions' existe (DB v3). Antes deste fix, esta
  // função nunca encontrava a tabela e o agente "esquecia" tudo o que
  // tinha sido ensinado sempre que o worker reiniciava.
  async function warmup() {
    try {
      ...
      console.log('[PropStore] Warmup:', _props.size, 'proposições carregadas do disco');
    } catch(e) {
      console.warn('[PropStore] Warmup falhou:', e.message);
    }
  }
  ...
  return {
    ...
    runInference, decay, warmup, stats, all, getActiveConflict, retract, teachStructured,
...
const PropResponseBuilder = (() => {
  ...
  function propToSentence(prop, lang) {
    ...
    // Reconstrói o qualifier na superfície textual: "aplicação de calor"
    // em vez de apenas "aplicação" — preserva a informação capturada pelo
    // detector de sintagma composto.
    const subjWithQual = prop.qualifier
      ? (lang === 'pt' ? `${subj} de ${prop.qualifier}` : `${subj} of ${prop.qualifier}`)
      : subj;

    if (!prop.object) {
      return `${subjWithQual} ${neg}${prop.relationLabel}.`;
    }
    return `${subjWithQual} ${neg}${prop.relationLabel} ${prop.object}.`;
  }
...
// ── PORTÃO DE HONESTIDADE ──────────────────────────────────────────────────
// Antes de cair no fallback geométrico (KD-Tree + Planner), verifica se o
// input é genuinamente desconhecido — nenhum dos seus tokens significativos
// existe no PropositionStore nem no grafo de nós com uso real. Se for
// desconhecido, recusa responder com confiança em vez de costurar conceitos
// geometricamente próximos mas semanticamente irrelevantes.
// Resolve: "Why" → "Fogo queima O fogo é energia." (alucinação por
// proximidade geométrica). Agora devolve honestamente "não sei".
function isInputKnown(tokens) {
  const meaningful = (tokens || []).filter(t => !t.roles?.isStop && (t.semanticWeight||0) > 0.3);
  ...
  for (const t of meaningful) {
    ...
    // 1. Existe como sujeito ou objecto de alguma proposição (mesmo deprecated/disputed)
    const bySubj = PropositionStore.queryBySubject(form, 0, true);
    const byObj  = PropositionStore.queryByObject(form, 0, true);
    if (bySubj.length > 0 || byObj.length > 0) return true;

    // 2. Existe no grafo de nós com utilização real (fireCount > 0)
    let foundInGraph = false;
    brain.nodes.forEach(node => {
      if (foundInGraph) return;
      const lex = node.signature?.lexical?.toLowerCase() || '';
      if (lex.includes(form.toLowerCase()) && (node.fireCount || 0) > 0) foundInGraph = true;
    });
    if (foundInGraph) return true;
  }
  return false;
}
...
(function patchSynthesizerWithProps() {
  ...
  ResponseSynthesizer.synthesize = function(opts) {
    ...
    // ── PORTÃO DE HONESTIDADE ────────────────────────────────────────────────
    // Sem proposição directa E o input não é reconhecido em lugar nenhum
    // do sistema → recusa, não alucina por proximidade geométrica.
    if (!isInputKnown(tokens)) {
      return {
        text: lang === 'pt'
          ? 'Não sei. Não tenho informação sobre isto ainda.'
          : "I don't know. I don't have information about this yet.",
        lang,
        intent:        'unknown',
        regime:        opts.regime || 'STABLE',
        resonance:     0,
    ...
    // Só chega aqui se pelo menos UM token do input é genuinamente conhecido
  ...
  console.log('[AIO-Patch] ResponseSynthesizer patchado com PropositionStore + portão de honestidade.');
})();
